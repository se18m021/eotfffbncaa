...
internal class KotlinMetricCalculatorLCAMTest : AbstractMetricCalculatorTest(
    metricName = "LCAM",
    resourcesFolder = "kotlin/lcam/",
    ...
    @Test
    fun `(LcamB) LCAM for class with single field and reference in primary constructor`() {
        assertMetric("LcamB", 1.0 - (3.0 / 4.0))
    }

    @Test
    fun `(LcamB2) LCAM for class with single field and reference in primary constructor via 'init'`() {
        assertMetric("LcamB2", 1.0 - (3.0 / 4.0))
    }

    @Test
    fun `(LcamB3) LCAM for class with single field and reference in primary constructor via direct assignment`() {
        assertMetric("LcamB3", 1.0 - (3.0 / 4.0))
    }
